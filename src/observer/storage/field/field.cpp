...
void Field::set_int(Record &record, int value)
{
  ...
  char *field_data = record.get_field_data(field_->id());
  memcpy(field_data, &value, sizeof(value));
}

int Field::get_int(const Record &record)
{
  Value value(field_->type(), const_cast<char *>(record.get_field_data(field_->id())), field_->len());
  return value.get_int();
}

const char *Field::get_data(const Record &record)
{
  return record.get_field_data(field_->id());
}