...
public:
  ...
  {
    set_variable_.name = set_variable.name;
    set_variable_.value = set_variable.value;
  ...
  const char *var_name() const { return set_variable_.name[0].c_str(); }
  const Value &var_value() const { return set_variable_.value[0]; }
