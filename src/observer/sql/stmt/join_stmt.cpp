  ...
  for (size_t i = 0; i < join_sql.join_list.size() / 2; i++) {
    ...
    if(!join_sql.join_list[i * 2 + 1].empty()) {
      if(table_map.find(join_sql.join_list[i * 2 + 1]) == table_map.end()){
        ...
        LOG_ERROR("name %s refers to multiple table", join_sql.join_list[i * 2 + 1].c_str());
        return RC::INVALID_ARGUMENT;
      }
    } else {
      if(table_map.find(join_sql.join_list[i * 2]) == table_map.end()){
        table_map[join_sql.join_list[i * 2]] = table;
      } else {
