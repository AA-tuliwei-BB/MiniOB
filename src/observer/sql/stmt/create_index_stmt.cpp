...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  for(auto &it : create_index.attribute_name) {
    const FieldMeta *tmp = table->table_meta().field(it.c_str());
    if (nullptr == tmp) {
      LOG_WARN("no such field in table. db=%s, table=%s, field name=%s", 
              db->name(), table_name, it.c_str());
      return RC::SCHEMA_FIELD_NOT_EXIST;   
    }
    field_meta.push_back(*tmp);
  }
  ...
  stmt = new CreateIndexStmt(table, std::move(field_meta), create_index.index_name, create_index.unique);
  return RC::SUCCESS;
}
